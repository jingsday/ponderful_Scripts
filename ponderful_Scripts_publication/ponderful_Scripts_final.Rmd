...
```{r corr per region}
# Define the function
model_df$Pastures.and.open.nature_500.s
screen_corr <- function(country, threshold = 0.5) {
  ...
  selected_vars <- df[, c('Cropland_500.s','bio12.s','bio7.s')]
...
high_correlations <- screen_corr(country = "Turkey")
high_correlations
```
...
tp_glm_analysis <- function(country, K) {
  ...
  initial_model <- glm(formula = TP~   Natural_5.s +Cropland_500.s +
                            Pastures.and.open.nature_500.s +
                             Animals_cont.s +  Depth.s,
...
summary(tp_belgium_test)
tab_model(tp_belgium_sig_model,transform = NULL,show.intercept = TRUE)
tp_belgium_test

df <- model_df[model_df$Country =='Belgium',]
glmnet()
check <-cv.glm(data = model_df[model_df$Country == "Belgium", ], glmfit = tp_belgium_test, K = 5)
check$delta[2]
tab_model(tp_belgium_test,transform = NULL,show.intercept = TRUE)

hist(model_df[model_df$Country =='Belgium',]$TP)
model_df$Pastures.and.open.nature_500.s
library(MASS)

stepAIC(glm(formula = TP ~ Natural_5.s +Cropland_500.s+  Animals_cont.s + Pastures.and.open.nature_500.s+
    Depth.s + bio1.s+bio12.s, family = Gamma(link = "log"), data = model_df[model_df$Country=='Belgium',]), direction = "both", k = log(30))
...
tp_spain_results_test <- glm(formula = TP~  Natural_5.s +  Cropland_500.s +
                            Pastures.and.open.nature_500.s +
                             Animals_cont.s +  Depth.s +bio1.s+bio12.s,
                       family = Gamma(link = "log"),
                       data = model_df[model_df$Country=='Spain',])
tab_model(tp_spain_results_test,transform = NULL,show.intercept = TRUE)
...
#
test <- glm(formula = TP ~    Depth.s, family = Gamma(link = "log"), 
    data = df)

test <- glm(formula = TP ~   Natural_5.s     +Pastures.and.open.nature_500.s+Animals_cont.s+bio7.s, family = Gamma(link = "log"), 
    data = df)

tab_model(test,transform = NULL,show.intercept = TRUE)
check <- cv.glm(test,data=df)
check$delta[2]
```
...
initial_model <- glm(formula = log_TN~ Natural_5.s +  
                       Pastures.and.open.nature_500.s +
                       Animals_cont.s + Depth.s,
                     data = df)
...
table(df$Country)
test <- glm(formula = log_TN~   Cropland_500.s+bio12.s, data = df)
tab_model(test,transform = NULL,show.intercept = TRUE)
...
tp_Uruguay_sig_model <- glm(formula = TP ~ Natural_5.s + Cropland_500.s   + Animals_cont.s + 
    Area.s + bio1.s, family = Gamma(link = "log"), data = model_df[model_df$Country=='Uruguay',])

summary(tp_Uruguay_sig_model)
```
