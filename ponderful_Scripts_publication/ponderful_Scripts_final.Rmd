...
dom_landcover_nov[dom_landcover_nov$Country %in% c('Switzerland','Belgium', 'Denmark', 'UK'), "altregion"] <- 'Atlantic'
...
plots_TN <- visreg(TN_full_model, gg = TRUE,partial=TRUE,scale='response')
plots_TP <- visreg(TP_full_model, gg = TRUE,partial=TRUE,scale='response')
...
TN_euromedi_mixed_1 <-lmer(formula = log_TN ~  Natural_5.s + Cropland_500.s +
                            Pastures.and.open.nature_500.s +
                             Animals_cont.s +Depth.s +
                             bio1.s+bio7.s+bio12.s+
                                 (1|Country) + (1|Country:Pondscape) , data = model_df,na.action = "na.fail") 
summary(TN_euromedi_mixed_1)
tab_model(TN_euromedi_mixed_1,show.intercept = TRUE,transform = NULL,digits.re = 4)
...
#TP 
TP_euromedi_mixed_1 <-glmer(formula =TP ~   Natural_5.s+Cropland_500.s +Animals_cont.s +
...
tab_model(TP_euromedi_mixed_1,show.intercept = TRUE,transform = NULL)#,file = paste0(outwdir,'TP_glmm.doc'))
#sig
...
var_components <- as.data.frame(VarCorr(test2))
...
variance_plot
ggsave(paste0(outwdir,'TP_glmm_partition.png'),plot=variance_plot, width = 8, height = 5.5, dpi = 300)
```
...
tab_model(
tp_alt_selected,show.intercept = TRUE,transform = NULL,digits.re = 3)
...
#random effect comparison
test1 <-glmer(TP~ Natural_5.s+Cropland_500.s +Animals_cont.s +
                              bio1.s + bio12.s+Depth.s+(1|altregion:Pondscape), family = Gamma(link = "log"), 
               data = model_df)
test2 <-glmer(TP~ Natural_5.s+Cropland_500.s +Animals_cont.s +
                              bio1.s + bio12.s+Depth.s+(1|altregion), family = Gamma(link = "log"), 
               data = model_df)
anova(tp_alt_selected,test1)
anova(tp_alt_selected,test2,test1)
anova(test1,test2)
