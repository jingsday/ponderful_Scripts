...
high_correlations <- screen_corr(country = "Switzerland")
high_correlations
```
...
TP_euromedi_linear_model <- glm(TP ~ Natural_5.s+Cropland_500.s + Pastures.and.open.nature_500.s + 
                             Animals_cont.s + Depth.s + bio1.s+bio7.s+bio12.s, family = Gamma(link = "log"),
                           data = model_df)
...
tp_glm_analysis <- function(country, K) {
  ...
  initial_model <- glm(formula = TP~ Animals_cont.s + bio1.s + Cropland_500.s + Depth.s + 
  Natural_5.s + Pastures.and.open.nature_500.s, 
