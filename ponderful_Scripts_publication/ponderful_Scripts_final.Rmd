...
plots_TP <- visreg(TP_euromedi_model_selected, gg = TRUE)

# Arrange plots for each model into separate combined plots
combined_plot_TN <- wrap_plots(plots_TN) + 
  plot_annotation(title = "Coefficients of Predictors in TN Model")

combined_plot_TP <- wrap_plots(plots_TP) + 
  plot_annotation(title = "Coefficients of Predictors in TP Model")

# Combine both TN and TP plots into a single layout
combined_plot <- (combined_plot_TN | combined_plot_TP)
combined_plot
# Save the combined plot as a PNG file
output_path <- paste0(outwdir, "TN_TP_coefficients_comparison.png")
ggsave(output_path, plot = combined_plot, width = 12, height = 6, dpi = 300)
```
