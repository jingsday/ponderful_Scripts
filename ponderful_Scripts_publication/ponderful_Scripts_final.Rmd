...
TN_euromedi_mixed_model<- lmer(log_TN ~ Natural_5.s + Aquatic_500.s + Cropland_500.s +
                            Pastures.and.open.nature_500.s +
                            Urban_500.s + Animals_cont.s + Area.s + Depth.s + Hydeoperiod_length.s+
                            bio1.s +bio7.s+ (1 | Country) , 
                          data = model_df)
summary(TN_euromedi_mixed_model)

model_set<- dredge(TN_euromedi_mixed_model,m.min=1,m.max=4)

get.models(model_set, 1)[[1]]

step(TN_euromedi_mixed_model)

TN_euromedi_mixed_model_selected <- lmer(log_TN ~ Depth.s + (1 | Country),
                                         data=model_df)
summary(TN_euromedi_mixed_model_selected)
tab_model(TN_euromedi_mixed_model_selected,show.intercept = TRUE,transform = NULL)

test1 <-lmer(log_TN ~  Cropland_500.s +
                             Animals_cont.s +  Depth.s 
                       +(1 | Country) , 
                          data = model_df)
tab_model(test1,show.intercept = TRUE,transform = NULL)

anova(test1,TN_euromedi_mixed_model_selected)
anova(test1,TN_euromedi_mixed_model)

summary(test1)
AIC(test1,TN_euromedi_mixed_model_selected)

#GLMM country and landcover
TN_euromedi_c_dom_model<- lmer(log_TN ~ Natural_5.s + Aquatic_500.s + Cropland_500.s +
                            Pastures.and.open.nature_500.s +
                            Urban_500.s + Animals_cont.s + Area.s + Depth.s +Hydeoperiod_length.s+
                            bio1.s +bio7.s+ (1 | Country)+ (1 | Country:dominant_landcover), 
                          data = model_df) #singular

model_set<- dredge(TN_euromedi_c_dom_model,m.min=1)
...
model_set<- dredge(TN_euromedi_pond_mixed_model,m.min=4)
get.models(model_set, 1)[[1]]

selected <- lmer(log_TN ~ Animals_cont.s + bio7.s + Cropland_500.s + Depth.s +      
                   (1 | Pondscape:dominant_landcover) , 
                          data = model_df)
tab_model(selected,show.intercept = TRUE,transform = NULL)
...
model_set<- dredge(TN_euromedi_dom_mixed_model,m.min=4)
get.models(model_set, 1)[[1]]

selected <- lmer(log_TN ~ Animals_cont.s + bio7.s + Cropland_500.s + Depth.s +      (1 | dominant_landcover:Pondscape),data=model_df)
...
model_set<- dredge(TN_euromedi_altregion_mixed_model,m.min=4)
get.models(model_set, 1)[[1]]
...
model_set<- dredge(TN_euromedi_altregion_nest_mixed_model,m.min=5)
get.models(model_set, 1)[[1]]

test <- lmer(log_TN ~ Animals_cont.s + bio7.s + Cropland_500.s + Depth.s +  
    Pastures.and.open.nature_500.s + (1 | altregion:dominant_landcover),data=model_df)
test1 <- glm(log_TN ~ Animals_cont.s + bio7.s + Cropland_500.s + Depth.s +  
    Pastures.and.open.nature_500.s ,data=model_df)
...
TP_euromedi_model_selected <-  glm(formula = TP ~ Animals_cont.s + bio1.s + Cropland_500.s + 
    Depth.s + Natural_5.s + Pastures.and.open.nature_500.s + 
    1, family = Gamma(link = "log"), data = model_df)
