...
TN_euromedi_mixed_5 <-lmer(formula = log_TN ~  Animals_cont.s + bio7.s + Depth.s+Cropland_500.s+
                                   (1|dominant_landcover)  , data = model_df,na.action = "na.fail") 

summary(TN_euromedi_mixed_5)
tab_model(lmer(formula = log_TN ~  Animals_cont.s +Depth.s+Cropland_500.s+
                                   (1|dominant_landcover)  , data = model_df,na.action = "na.fail"),show.intercept = TRUE,transform = NULL)


```

```{r (E) (1|Dominant_landuse:Pondscape)}

TN_euromedi_mixed_6 <-lmer(formula = log_TN ~  Animals_cont.s + bio7.s + Depth.s+Cropland_500.s+
                                  (1|dominant_landcover:Pondscape)  , data = model_df,na.action = "na.fail") 
summary(TN_euromedi_mixed_6)

TN_euromedi_mixed_6_selected <- lmer(formula = log_TN ~  Animals_cont.s + Depth.s+Cropland_500.s+
                                  (1|dominant_landcover:Pondscape)  , data = model_df,na.action = "na.fail") 

tab_model(TN_euromedi_mixed_6_selected,show.intercept = TRUE,transform = NULL)
...
```{r TN GLMM Section III: (A): (cropland|Country) + (animals|Country)+ (1|Country:Pondscape)}
