...
##Alternative categories##Alternative categories##Alternative categories
...
output_path <- paste0(outwdir, "SM1_distribution.png")
...
```{r}
global_x_range <- c(-3.0, 3.0)
global_y_range <- c(-3.5,1)
...
all_plots <- c( plots_TN_minimal[1:2],list(blank_panel),plots_TP_minimal[c(1:5, 7)])
all_plots
...
full_combined_plot
output_path <- paste0(outwdir, "full_glmm_alt_coefficients_comparison.png")
ggsave(output_path, plot = full_combined_plot, width = 36, height = 33, dpi = 300)
...
TN_mixed_1_selected <-lmer(formula = log_TN ~  bio12.s +  Depth.s+Cropland_500.s+
                                 (1|Country) + (1|Country:Pondscape) , data = model_df,na.action = "na.fail") 
summary(TN_mixed_1_selected)

#Depth outliers removal
TN_mixed_1_selected <-lmer(formula = log_TN ~   Depth.s+Cropland_500.s+Animals_cont.s+
                                 (1|Country) + (1|Country:Pondscape) , data = model_df,na.action = "na.fail") 
summary(TN_mixed_1_selected)

anova(TN_mixed_1,TN_mixed_1_selected)

tab_model(TN_mixed_1_selected,show.intercept = TRUE,transform = NULL,digits.re = 4)#,file=paste0(outwdir,'TN_GLMM_depth_outliers_removed.doc'))
...
TP_euromedi_mixed_1_selected <-glmer(formula =TP ~   Natural_5.s+Cropland_500.s +Animals_cont.s +
                             bio1.s + bio12.s+(1|Country) +Depth.s+(1|Country:Pondscape) , data = model_df,
                             family =Gamma(link = "log"), na.action = "na.fail")
tab_model(TP_euromedi_mixed_1_selected,show.intercept = TRUE,transform = NULL,digits.re = 4)#,file = paste0(outwdir,'TP_glmm.doc'))
...
tab_model(TP_euromedi_mixed_1_selected,show.intercept = TRUE,transform = NULL)#,file = paste0(outwdir,'TP_glmm_depth_outlier_removed.doc'))

anova(TP_euromedi_mixed_1,test1)
anova(test1,test2,TP_euromedi_mixed_1_selected)
anova(test1,test2)
anova(test1,TP_full_model)
anova(test2,TP_full_model)
anova(test2,TP_euromedi_mixed_1_selected)
...
tab_model(test_2,show.intercept = TRUE,transform = NULL,digits.re = 4)#,file = paste0(outwdir,'TN_euromedi_glmm.doc'))
