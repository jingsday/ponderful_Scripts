...
# Part I: Prep starting from full
...
```{r TP Mixed }
#bioregion as random effect gives marginal difference
input <- 'Uruguay'
df <-full_df_standardized[full_df_standardized$Country != input,][,c('Country','Pondscape','bioregion','TP','Natural_5_qt',  'Cropland_500_qt', 'Urban_500_qt',
                                                'Depth.t','Forest_500_qt','Area.t','Pastures.and.open.nature_500_qt',
                                                'Aquatic_500_qt','Animals_cont.t',
                                                'bio1.t','bio7.t','bio5.t','bio12.t')]
...
df <-na.omit(df)
dim(df)
#selected vars from glm(also full model)
tp_mixed_euromedi_linear_model <-glmer(formula = TP ~ Cropland_500_qt +
                                         Pastures.and.open.nature_500_qt + (1|Country) + (1 | Country:Pondscape)+
                                        Animals_cont.t + Depth.t + bio1.t, family = Gamma(link = "log"), data = df)
tab_model(tp_mixed_euromedi_linear_model,file=paste0(outwdir,'tp_mixed_euromedi_nested_model.doc'))
...
test<-glmer(formula = TP ~ Cropland_500_qt +
                                         Pastures.and.open.nature_500_qt + (1|Country)+
                                        Animals_cont.t + Depth.t , family = Gamma(link = "log"), data = df)
test1 <-glmer(formula = TP ~
                                         Pastures.and.open.nature_500_qt + (1|Country)+
                                        Animals_cont.t + Depth.t+bio1.t , family = Gamma(link = "log"), data = df)
test2<-glmer(formula = TP ~ Cropland_500_qt +
                                          (1|Country)+
                                        Animals_cont.t + Depth.t + bio1.t, family = Gamma(link = "log"), data = df)
test3<-glmer(formula = TP ~ Cropland_500_qt +
                                         Pastures.and.open.nature_500_qt + (1|Country)+
                                       Depth.t + bio1.t, family = Gamma(link = "log"), data = df)

test4<-glm(formula = TP ~ Cropland_500_qt +
                                         Pastures.and.open.nature_500_qt +
                                        Animals_cont.t + Depth.t + bio1.t, family = Gamma(link = "log"), data = df)

test5 <-glmer(formula = TP ~ Cropland_500_qt +
                                         Pastures.and.open.nature_500_qt + (1|Country)+
                                        Animals_cont.t + bio1.t, family = Gamma(link = "log"), data = df)

AIC(test5,tp_mixed_euromedi_linear_model)

model <- glmer(TP ~ Cropland_500_qt+ bio1.t + Pastures.and.open.nature_500_qt +
                 Animals_cont.t + Depth.t + (1 | Pondscape),  data = df,  family = Gamma(link = "log"))
summary(model)
tab_model(model)
AIC(model,TP_euromedi_gam_model_gamma_selected)
...
AIC(tp_mixed_euromedi_linear_model)
test1 <- glmer(formula = TP ~ Pastures.and.open.nature_500_qt + (1|Country) + (1 | Country:Pondscape)+Animals_cont.t + Depth.t + bio1.t, family = Gamma(link = "log"), data = df)


test2 <- glmer(formula = TP ~
                                          (1|Country) + (1 | Country:Pondscape)+
                                        Animals_cont.t + Depth.t , family = Gamma(link = "log"), data = df)
summary(test2)
AIC(test2)
tab_model(test2,file=paste0(outwdir,'tp_mixed_euromedi_nested_model_AIC_selected.doc'))

```

```{r TN Mixed selected vars from glm}
input <- 'Uruguay'

df <-full_df_standardized[full_df_standardized$Country != input,][,c('Country','bioregion','Pondscape','log_TN','Natural_5_qt',  'Cropland_500_qt', 'Urban_500_qt',
                                                'Depth.t','Forest_500_qt','Area.t','Pastures.and.open.nature_500_qt',
                                                'Aquatic_500_qt','Animals_cont.t',
                                                'bio1.t','bio7.t','bio5.t','bio12.t')]
...
#selected vars+Country|1
TN_mixed_log_euromedi_linear_model <- lmer(formula = log_TN ~ Cropland_500_qt + Pastures.and.open.nature_500_qt + 
    Animals_cont.t + Area.t + Depth.t + bio7.t+(1|bioregion), data = df)


TN_mixed_log_euromedi_linear_model <- lmer(formula = log_TN ~ Cropland_500_qt + Pastures.and.open.nature_500_qt + 
    Animals_cont.t + Area.t + Depth.t + bio7.t+(1|Country), data = df)

summary(TN_mixed_log_euromedi_linear_model)
tab_model(TN_mixed_log_euromedi_linear_model)#,file= paste0(outwdir, 'TN_mixed_log_euromedi_linear_model.doc'))


test <- lmer(formula = log_TN ~ Cropland_500_qt + Pastures.and.open.nature_500_qt + 
    Animals_cont.t + Area.t + Depth.t + bio7.t+(1+ bioregion|Country), data = df)


test <- lmer(formula = log_TN ~ Cropland_500_qt +  Area.t + Depth.t +(1|Country), data = df)
summary(test)
tab_model(test)
#all vars
TN_mixed_log_euromedi_full <- lmer(log_TN ~ Natural_5_qt + Aquatic_500_qt + Cropland_500_qt +
                            Pastures.and.open.nature_500_qt +
                             Urban_500_qt + Animals_cont.t + Area.t + Depth.t +
                             bio1.t+bio7.t+(1|Country),
                           data = df)
summary(TN_mixed_log_euromedi_full)


library(lmerTest)

step(TN_mixed_log_euromedi_full)


TN_mixed_log_euromedi_full_selected <- lmer(log_TN ~ Area.t + Depth.t + (1 | Country),data=df)
summary(TN_mixed_log_euromedi_full_selected)

tab_model(TN_mixed_log_euromedi_full_selected,file= paste0(outwdir, 'TN_mixed_log_euromedi_full_selected.doc'))


#nested pondscape
TN_mixed_log_euromedi_nested_model <- lmer(formula = log_TN ~ Cropland_500_qt + Pastures.and.open.nature_500_qt + 
    Animals_cont.t + Area.t + Depth.t + bio7.t+(1|Country) + (1 | Country:Pondscape), data = df)

tab_model(TN_mixed_log_euromedi_nested_model)
tab_model(TN_mixed_log_euromedi_nested_model, file=paste0(outwdir,'TN_mixed_log_euromedi_nested_model.doc'))


TN_mixed_log_euromedi_nested_model_full <- lmer(log_TN ~ Natural_5_qt + Aquatic_500_qt + Cropland_500_qt +
                            Pastures.and.open.nature_500_qt +
                             Urban_500_qt + Animals_cont.t + Area.t + Depth.t +
                             bio1.t+bio7.t+(1|Country) + (1 | Country:Pondscape),
                           data = df)
summary(TN_mixed_log_euromedi_nested_model_full)


step(TN_mixed_log_euromedi_nested_model_full)


TN_mixed_log_euromedi_nested_model_selected <- lmer(log_TN ~ Depth.t + (1 | Country:Pondscape),data=df)
summary(TN_mixed_log_euromedi_nested_model_selected)
tab_model(TN_mixed_log_euromedi_nested_model_selected, file=paste0(outwdir,'TN_mixed_log_euromedi_nested_model_selected.doc'))
```



```{r 04/10/2024 afternoon}
model2 <- lmer(log_TN ~ Cropland_500_qt + Pastures.and.open.nature_500_qt +
                  Animals_cont.t + Area.t+ Depth.t + bio7.t+ (1 + 1| Country),  data = df )
summary(model2)
random_intercepts <- ranef(model2)$Country
random_intercepts_df <- as.data.frame(random_intercepts)
random_intercepts_df$Country <- rownames(random_intercepts_df)
colnames(random_intercepts_df)[1] <- "Intercept"

png(paste0(outwdir,"TN_Country_intercepts.png"), height = 7, width = 5, units = "in", res = 300)
ggplot(random_intercepts_df, aes(x = reorder(Country, Intercept), y = Intercept)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  labs(x = "Country", y = "Random Intercept (Difference from Overall Intercept)",
       title = "Differences in Country Intercepts") +
  theme_minimal() +
  theme(
...
df <-full_df_standardized[full_df_standardized$bioregion == input,][,c('log_TP','TP',
                                                     'Natural_5', 
...
df <-na.omit(df)
cvfit<-cv.glm(df,TP_medi_linear_model_selected,K=6)
print(cvfit$delta)

plot.new()
plot(cvfit$delta, type = "b", pch = 19, col = "blue",
     xlab = "Cross-Validation Type", ylab = "CV Error",
     main = "Cross-Validation Error Estimates")
abline(h = mean(cvfit$delta), col = "red", lty = 2)  # Optional: add a horizontal line for mean error
...
combined_plot <- wrap_plots(plots)
combined_plot
ggsave(filename = paste0(outwdir, 'tp_range_glm_gamma_predictors.png'), 
       plot = combined_plot, 
       width = 12, height = 10, units = "in", dpi = 600)

#Diagnostic plots
diagnostic_plots <- appraise(TP_range_linear_model_selected)
ggsave(filename = paste0(outwdir, 'tp_range_glm_gamma_diagnostics.png'), 
       plot = diagnostic_plots, 
       width = 10, height = 10, units = "in",dpi = 600)
...
combined_plot <- wrap_plots(plots)
combined_plot
ggsave(filename = paste0(outwdir, 'TN_range_glm_gamma_predictors.png'), 
       plot = combined_plot, 
       width = 12, height = 10, units = "in", dpi = 600)

#Diagnostic plots
diagnostic_plots <- appraise(TN_range_linear_model_selected)
ggsave(filename = paste0(outwdir, 'TN_range_glm_gamma_diagnostics.png'), 
       plot = diagnostic_plots, 
       width = 10, height = 10, units = "in",dpi = 600)
...
combined_plot <- wrap_plots(plots)
combined_plot
ggsave(filename = paste0(outwdir, 'TN_LOG_range_glm_gamma_predictors.png'), 
       plot = combined_plot, 
       width = 12, height = 10, units = "in", dpi = 600)

#Diagnostic plots
diagnostic_plots <- appraise(TN_log_range_linear_model_selected)
ggsave(filename = paste0(outwdir, 'TN_LOG_range_glm_gamma_diagnostics.png'), 
       plot = diagnostic_plots, 
       width = 10, height = 10, units = "in",dpi = 600)
```
...
#Part VII: TP models in countries

```{r Spain }
input <- 'Spain'
df <-full_df_standardized[full_df_standardized$Country == input,][,c('log_TP','TP',
                                                     'Natural_5', 
                                                     'Cropland_500', 'Urban_500',  'Depth','Area',
                                                     'Pastures.and.open.nature_500',
                                                     'Aquatic_500','Animals_cont', 'bio1',
...
corrplot(cor(df[c('TP','Cropland_500_qt', 'Pastures.and.open.nature_500_qt' , 'Animals_cont.t' ,'Depth.t' , 'bio1.t')]),order='alphabet',type='upper',tl.srt=45, tl.col='black')

round(cor(df[c('Cropland_500_qt', 'Pastures.and.open.nature_500_qt' , 'Animals_cont.t' ,'Depth.t' , 'bio1.t')]),2)
round(cor(df),2)
corrplot(cor(df),type='upper',order='alphabet',tl.srt=45,tl.col ='black',)

#Model glm gamma
TP_glm_spain_gamma <- glm(formula = TP ~ Cropland_500_qt + Pastures.and.open.nature_500_qt + 
    Animals_cont.t + Depth.t + bio1.t, family = Gamma(link = "log"), 
    data = df)

summary(TP_glm_spain_gamma)
tab_model(TP_glm_spain_gamma)
tab_model(
  TP_glm_spain_gamma, 
  show.intercept = TRUE, 
  transform = NULL)
test <- glm(formula = TP ~ Cropland_500_qt  + 
    Animals_cont.t + Depth.t + bio1.t, family = Gamma(link = "log"), 
    data = df)
tab_model(test)
test1 <- glm(formula = TP ~ Cropland_500_qt  + 
    Animals_cont.t + Depth.t + bio1.t, family = Gamma(link = "log"), 
    data = df)
tab_model(test)


TP_glm_spain_gamma_selected <- glm(formula = TP ~ Cropland_500_qt  + Depth.t , family = Gamma(link = "log"), 
    data = df)
tab_model(TP_glm_spain_gamma_selected)

tab_model(test)
print(AIC(test1,test))
summary(test)
test_selected <-  glm(formula = log_TP ~ Cropland_500_qt + Animals_cont.t + Depth.t + bio1.t, data = df)
...
#Sum table
tab_model(TP_glm_spain_gamma,
          file = paste0(outwdir, 'tp_spain_gam_sum.doc'))

#Predictors
plots <- visreg(TP_glm_spain_gamma,gg=TRUE)
combined_plot <- wrap_plots(plots)
combined_plot
ggsave(filename = paste0(outwdir, 'tp_spain_gam_predictors_full.svg'), 
       plot = combined_plot, 
       width = 12, height = 10, units = "in", dpi = 600)

#Diagnostic plots
diagnostic_plots <- appraise(TP_glm_spain_gamma)
ggsave(file = paste0(outwdir, 'tp_spain_gam_diagnostics.svg'), 
       plot = diagnostic_plots, 
       width = 10, height = 10, units = "in",dpi = 600)
```
...
corrplot(cor(df[c('Cropland_500_qt', 'Pastures.and.open.nature_500_qt' , 'Animals_cont.t' ,'Depth.t' , 'bio1.t')]),order='alphabet',type='upper',tl.srt=45, tl.col='black')
...
#Model glm gamma
TP_glm_germany_gamma <- glm(formula = TP ~ Cropland_500_qt + Pastures.and.open.nature_500_qt + 
    Animals_cont.t + Depth.t + bio1.t, family = Gamma(link = "log"), 
    data = df)
tab_model(TP_glm_germany_gamma)
summary(TP_glm_germany_gamma)
test <- glm(formula = TP ~ Cropland_500_qt + Pastures.and.open.nature_500_qt + 
     Depth.t + bio1.t, family = Gamma(link = "log"), 
    data = df)
AIC(TP_glm_germany_gamma,test)
tab_model(test)
...
test2 <- glm(formula = TP ~ Cropland_500_qt + 
     Depth.t , family = Gamma(link = "log"), 
    data = df)


test <-  glm(formula = log_TP ~ Cropland_500_qt + Pastures.and.open.nature_500_qt +
    Animals_cont.t + Depth.t + bio1.t,
    data = df)
tab_model(test2)
step(test)
test_selected<-  glm(formula = log_TP ~ Cropland_500_qt + Animals_cont.t + Depth.t + 
    bio1.t, data = df)
tab_model(test_selected)
...
#Model glm gamma
TP_glm_denmark_gamma <- glm(formula = TP ~ Cropland_500_qt + Pastures.and.open.nature_500_qt + 
    Animals_cont.t + Depth.t + bio1.t, family = Gamma(link = "log"), 
    data = df)
tab_model(TP_glm_denmark_gamma)
summary(TP_glm_denmark_gamma)
test <- glm(formula = TP ~ Cropland_500_qt + Pastures.and.open.nature_500_qt + 
     Depth.t + bio1.t, family = Gamma(link = "log"), 
    data = df)
AIC(TP_glm_denmark_gamma,test)
...
TP_glm_denmark_gamma_selected <- glm(formula = TP ~ Cropland_500_qt + 
                                       Depth.t , family = Gamma(link = "log"), data = df)
...
tab_model(test)
step(test)
test_selected<-   glm(formula = log_TP ~ bio1.t, data = df)
tab_model(test_selected)

#OUTPUT
tab_model(TP_glm_denmark_gamma_selected,
          file = paste0(outwdir, 'tp_denmark_gam_sum.doc'))

#Predictors
plots <- visreg(TP_glm_denmark_gamma_selected,gg=TRUE)
combined_plot <- wrap_plots(plots)
combined_plot
ggsave(filename = paste0(outwdir, 'tp_denmark_gam_predictors_full.svg'), 
       plot = combined_plot, 
       width = 12, height = 10, units = "in", dpi = 600)

#Diagnostic plots
diagnostic_plots <- appraise(TP_glm_denmark_gamma_selected)
ggsave(file = paste0(outwdir, 'tp_denmark_gam_diagnostics.svg'), 
       plot = diagnostic_plots, 
       width = 10, height = 10, units = "in",dpi = 600)
```
...
#Model glm gamma
TP_glm_uruguay_gamma <- glm(formula = TP ~ Cropland_500_qt + Pastures.and.open.nature_500_qt + 
    Animals_cont.t + Depth.t + bio1.t, family = Gamma(link = "log"), 
    data = df)

summary(TP_glm_uruguay_gamma)
tab_model(TP_glm_uruguay_gamma)
tab_model(
  TP_glm_uruguay_gamma, 
  show.intercept = TRUE, 
  transform = NULL)
test <- glm(formula = TP ~ Cropland_500_qt  + 
    Animals_cont.t + Depth.t + bio1.t, family = Gamma(link = "log"), 
...
#Model glm gamma
TP_glm_uk_gamma <- glm(formula = TP ~ Cropland_500_qt + Pastures.and.open.nature_500_qt + 
    Animals_cont.t + Depth.t + bio1.t, family = Gamma(link = "log"), 
    data = df)

summary(TP_glm_uk_gamma)
tab_model(TP_glm_uk_gamma)
tab_model(
  TP_glm_uk_gamma, 
  show.intercept = TRUE, 
  transform = NULL)
...
TP_glm_spain_gamma_selected <- glm(formula = TP ~ Cropland_500_qt  + Depth.t , family = Gamma(link = "log"), 
    data = df)
tab_model(TP_glm_uk_gamma_selected)
...
#Model glm gamma
TP_glm_belgium_gamma <- glm(formula = TP ~ Cropland_500_qt + Pastures.and.open.nature_500_qt + 
    Animals_cont.t + Depth.t + bio1.t, family = Gamma(link = "log"), 
    data = df)

summary(TP_glm_belgium_gamma)
tab_model(TP_glm_belgium_gamma)
tab_model(
  TP_glm_belgium_gamma, 
  show.intercept = TRUE, 
  transform = NULL)
test <- glm(formula = TP ~ Cropland_500_qt  + 
    Animals_cont.t + Depth.t + bio1.t, family = Gamma(link = "log"), 
...
```{r}
# Load necessary library for tab_model
library(sjPlot)

# Define the function for backward stepwise model selection
stepwise_selection <- function(data, full_formula, fam, lk, min_r2_ratio = 0.9) {
  ...
  df <-full_df_standardized[full_df_standardized$Country == input,][,c('log_TP','TP',
                                                     ...
                                                     'Aquatic_500_qt','Animals_cont.t', 'bio1.t')]
  #starting models
  starting_model <- glm(formula = res_var ~ Cropland_500_qt + Pastures.and.open.nature_500_qt + 
    Animals_cont.t + Depth.t + bio1.t, family = fam(link = paste0(link)), 
    data = df)
  
  starting_aic <- AIC(starting_model)
  starting_r2 <- summary(starting_model)$r.squared
  
  cat("Starting Model AIC:", starting_aic, "\n")
  cat("Starting Model R²:", starting_r2, "\n")
  
  # Initialize a list to store models and their stats
  models <- list()
  models[[1]] <- list(model = starting_model, aic = starting_aic, r2 = starting_r2)
  
  # Extract variables from the full formula
  terms <- all.vars(full_formula)[-1]  # Exclude response variable
  ...
  for (i in seq_along(terms)) {
    ...
    # Generate models by removing one variable at a time
    for (j in 1:length(terms)) {
      # Create new formula by excluding one variable
      current_terms <- terms[-j]
      current_formula <- as.formula(paste("res_var ~", paste(current_terms, collapse = " + ")))
      
      # Fit the new model
      test_model <- glm(formula = current_formula, family = fam(link = link), data = data)
      
      # Calculate AIC and R²
      test_aic <- AIC(test_model)
      test_r2 <- calc_r2(test_model)
      
      # Compare AIC and R² to keep or discard the model
      if (test_aic < starting_aic && test_r2 >= starting_r2 * min_r2_ratio) {
        cat("\nBetter model found by removing", terms[j], "\n")
        cat("New AIC:", test_aic, "New R²:", test_r2, "\n")
        
        models[[length(models) + 1]] <- list(model = test_model, aic = test_aic, r2 = test_r2)
        
        # Update starting values for next iteration
        starting_aic <- test_aic
        starting_r2 <- test_r2
        
        # Print summary and tab_model
        print(summary(test_model))
        tab_model(test_model)
        
        # Break out of the loop if a better model is found
        break
      }
    }
  }
...
# Example usage:
# The full formula
full_formula <- TP ~ Cropland_500_qt + Pastures.and.open.nature_500_qt + Animals_cont.t + Depth.t + bio1.t

# Run the stepwise selection
best_models <- stepwise_selection(df, full_formula, fam = Gamma, link = "log")
```
