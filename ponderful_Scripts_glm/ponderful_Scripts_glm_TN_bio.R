...
##Check distribution of TP per ecoclimate region
...
full_df_standardized_TP <- df %>%
  mutate(across(c(Natural_5_qt, Aquatic_500_qt, Cropland_500_qt, 
                  ...
                  bio6.t,bio7.t,
                  bio12.t,bio15.t,bio17.t), ~ scale(.)[, 1])) # Standardizing each column
...
unique(full_df_standardized_TP$Animals_cont.t)
...
gamm_slope <- gam(TP ~ s(Natural_5_qt) + s(Aquatic_500_qt) + 
                    s(Cropland_500_qt) + s(Forest_500_qt) + 
                    s(Pastures.and.open.nature_500_qt)+s(Urban_500_qt)+s(Animals_cont.t,k=7)+
