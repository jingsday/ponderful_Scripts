...
png(filename = paste0(outwdir, 'tp_euromedi_gam_predictors.png'), width = 35, height = 20, units = 'cm', res = 300) 

par(mfrow = c(2, 4), mar = c(4, 4, 2, 2), oma = c(2, 2, 2, 2), mgp = c(3, 1, 0))
...
```
```{r diagnostics, echo=FALSE, message=FALSE, warning=FALSE}
png(filename=paste0(outwdir, "tp_log_euromedi_glm_diagnostics.png"))#, units = "cm", res=600)
par(mfrow = c(2,2))  # 2x2 layout
#par(mar = c(4, 4, 2, 2))  # Adjust margins for individual plots (bottom, left, top, right)
par(oma = c(6,6,6,6))  # Increase outer margins to avoid cutting off plots
...
print(
png(filename=paste0(outwdir,"tp_log_euromedi_glm_predictors.png"), width = 40, height = 30, units = "cm",res=300)
par(mfrow=c(3,2))
par(mar = c(3.5, 3.5, 1, 1), mgp = c(2, 0.7, 0))
visreg(TP_log_euromedi_linear_model_selected,cex.axis = 1.5,  cex.lab = 1.5)
dev.off()
```
